...
function PWABadge() {
  ...
  const { user } = useAuth();
  const { canInstall, install, sessionPromptHidden, setSessionPromptHidden } = usePwa();
  ...
  // Only show the badge for:
  // 1. Critical SW updates (needRefresh)
  // 2. Offline readiness (offlineReady)
  // 3. INSTALL prompt ONLY IF:
  //    - Authenticated
  //    - Not hidden in this session
  //    - Browser says it's installable
  const isInstallPromptVisible = !!user && !sessionPromptHidden && canInstall;
  const showBadge = offlineReady || needRefresh || isInstallPromptVisible;
    ...
    <div className="PWABadge" role="alert" aria-labelledby="toast-message">
      <div className="fixed bottom-6 right-6 z-[100] p-4 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-xl shadow-2xl flex flex-col gap-3 max-w-sm animate-fade-in-up">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-1">
              {needRefresh ? 'New update available' : isInstallPromptVisible ? 'Install Mobile App' : 'App ready for offline'}
            </h3>
        ...
        <div className="flex justify-end gap-2 mt-2">
          ...
          {isInstallPromptVisible && !needRefresh && (
            <button
